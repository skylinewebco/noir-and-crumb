...
function Card({ p }) {
  ...
  return (
    ...
    >
      {/* badge + wishlist */}
      <div className="absolute top-3 inset-x-3 sm:top-4 sm:inset-x-4 z-10 flex items-start justify-between">
        {p.badge ? <span className="text-[0.56rem] sm:text-[0.62rem] tracking-luxe uppercase px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-black/40 backdrop-blur text-cream-100 border border-white/10">{p.badge}</span> : <span />}
        <button
          onClick={(e) => { e.stopPropagation(); toggleWishlist(p.id) }}
          className={`w-8 h-8 sm:w-9 sm:h-9 grid place-items-center rounded-full backdrop-blur border border-white/10 transition ${wished ? 'text-[var(--accent)] bg-black/40' : 'text-cream-100/80 bg-black/25 hover:text-[var(--accent)]'}`}
          aria-label="Save to wishlist"
        ><Icon.Heart width={16} height={16} /></button>
      </div>
      ...
      {/* info */}
      <div className="p-3.5 pt-2.5 sm:p-5 sm:pt-3">
        <div className="flex items-baseline justify-between gap-2">
          <h3 className="font-display text-lg sm:text-2xl leading-tight sm:leading-none text-cream-50">{p.name}</h3>
          <span className="text-cream-100 text-sm sm:text-base tabular-nums shrink-0">${p.price.toFixed(2)}</span>
        </div>
        <p className="text-cream-300/60 text-xs sm:text-sm mt-1 sm:mt-2 line-clamp-1">{p.tagline}</p>

        <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2 mt-3 sm:mt-4">
          <button onClick={quickAdd} className="btn btn-primary !py-2 sm:!py-2.5 !px-3 sm:!px-4 justify-center text-[0.68rem] sm:text-[0.72rem]">
            <Icon.Plus width={14} height={14} /> Quick&nbsp;Add
          </button>
          <button onClick={(e) => { e.stopPropagation(); openProduct(p.id) }} className="btn btn-ghost !py-2 sm:!py-2.5 !px-3 sm:!px-4 !text-cream-100 border-white/15 text-[0.68rem] sm:text-[0.72rem] justify-center" aria-label="Customise">
            Customise
          </button>
        </div>
      </div>
  ...
  return (
    <section id="collection" className="relative py-24 sm:py-32">
      <div className="mx-auto max-w-[1400px] px-5 sm:px-8">
        ...
        {/* grid */}
        <div ref={revRef} key={filter} className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
